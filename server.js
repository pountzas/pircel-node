...
app.get('/houses', (req, res) => {
  ...
  if (offset + limit >= filteredHouses.length) {
    return res.status(400).json({ message: 'Invalid offset and limit' });
  }

  if (offset && limit) {
    const offsetLimitHouses = filteredHouses.slice(offset, offset + limit);

    res.json(offsetLimitHouses);
  }
